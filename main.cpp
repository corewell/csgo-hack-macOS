...
void Glow(mach_vm_address_t moduleStart, mach_vm_address_t glowStartAddress)
{
    for (int i = 0; i < 60; i++) {
...
int main(int argc, const char * argv[])
{
    ...
    while (mainPid != -1 && mainTask != 0)
    {
        ...
        iTeamNum = mem->read<int>(playerAddress + Offsets["m_iTeamNum"]);
        
        Glow(moduleStartAddress, glowObjectLoopStartAddress);
